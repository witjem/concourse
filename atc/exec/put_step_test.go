			...
			Context("when the inputs are detected", func() {
				...
				Context("when the params have maps and slices", func(){
					BeforeEach(func() {
						...
						putPlan.Params = atc.Params{"some-slice": [][]string{[]string{"some-source/source", "does-not-exist"}, []string{"does-not-exist-2"}}, "some-map": map[string]string{"key": "some-other-source/source"}}
					})
